...
class ResponsableDemandeController extends Controller
{
    ...
    /**
     * --------------------------------------------------------------------------
     * Notifier l'étudiant d'un changement de statut sur sa demande.
     * --------------------------------------------------------------------------
     */
    private function notifierEtudiant(DemandeStage $demande, string $titre, string $message): void
    {
        if (!$demande->candidat) {
            return;
        }
    ...
    public function refuser(Request $request, int $id)
    {
        ...
        $this->notifierEtudiant(
            $demande,
            'Demande refusée',
            "Votre demande de stage {$demande->numeroDemande} a été refusée." . (!empty($validated['motif']) ? " Motif : {$validated['motif']}" : '')
        );
    ...
    public function demanderInfos(Request $request, int $id)
    {
        ...
        $this->notifierEtudiant(
            ...
            "Le responsable a demandé des informations complémentaires pour votre demande {$demande->numeroDemande} : {$validated['message']}"
        );
